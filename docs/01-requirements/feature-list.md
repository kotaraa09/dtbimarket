...
Status: **updated 2026-09-03.** The M0 foundation and all of M1 are built. Update the status column as slices land; do not create a second list somewhere else.

The event enum is no longer a proposal: it now lives in `packages/shared/src/events.ts`, which is the source of truth, with a payload allowlist per event type in `event-payloads.ts`.
...
The canonical event enum lives in `packages/shared/src/events.ts`. This table and that file must agree; the file wins.
...
| ID | Feature | Actor | REQ | PB | Events | Pri | Status |
|---|---|---|---|---|---|---|---|
| FEAT-B1 | Store profile create and edit | Seller | REQ-B1 | PB-07 | `store.created`, `store.profile_updated` | Must | **done** |
| FEAT-B2 | Product create, edit, delete | Seller | REQ-B2 | PB-08 | `product.created`, `product.description_changed`, `product.deleted` | Must | **done** |
| FEAT-B3 | Product publish and unpublish | Seller | REQ-B2 | PB-09 | `product.published`, `product.unpublished` | Must | **done** |
| FEAT-B4 | Product photo add and remove | Seller | REQ-B3 | PB-10 | `product.photo_added`, `product.photo_removed` | Must | **done** |
| FEAT-B5 | Price and stock change | Seller | REQ-B4 | PB-11 | `product.price_changed`, `product.stock_changed` | Must | **done** |
...
| FEAT-E4 | Thai variant copy for EXP-001 — fixed templates filled from `metric_snapshot` (ADR-0002) | System | REQ-E1 | PB-23 | none — content, delivered by FEAT-E1 | Must | todo |
...
| ID | Feature | Actor | REQ | PB | Events | Pri | Status |
|---|---|---|---|---|---|---|---|
| FEAT-F1 | `Event` model, append-only guard, server-side emitter | System | REQ-F1, REQ-F2, REQ-F5 | PB-03 | infrastructure — carries every event above | Must | **done** |
...
| FEAT-F4 | Seed flagging and exclusion | System | REQ-F6 | PB-05 | none — internal | Must | **done** |
...
| FEAT-H3 | Core data model and seed data | System | REQ-D3, REQ-F6 | PB-02, PB-05 | none | Must | **done** |
